...
int ftplib::FtpAccess(const char *path, accesstype type, transfermode mode, ftphandle *nControl, ftphandle **nData)
{
	...
	if ((path == NULL) && ((type == ftplib::filewrite) || (type == ftplib::fileread) || (type == ftplib::filereadappend) || (type == ftplib::filewriteappend)))
	{
...
{
	vector<string> temp;
	return FtpXferEx(localfile, path, nControl, type, mode, temp, false);
}

int ftplib::FtpXferEx(const char *localfile, const char *path, ftphandle *nControl, accesstype type, transfermode mode, vector<string> &result, bool extended)
{
	int l, c;
	char *dbuf;
	FILE *local = NULL;
	...
	else
	{
		if (!extended)
		{
			...
			while ((l = FtpRead(dbuf, FTPLIB_BUFSIZ, nData)) > 0)
			{
				string remotePath = dbuf;
				string prefix = path;
				if(prefix != "" && prefix != "."){
					remotePath.erase(0,prefix.length()+1);
				}
				remotePath.erase(remotePath.length()-1,remotePath.length());				
				result.push_back(remotePath);	
			}
		}
	}
	free(dbuf);
...
int ftplib::NlstEx(const char *outputfile, const char *path,vector<string> &result)
{
	mp_ftphandle->offset = 0;
	return FtpXferEx(outputfile, path, mp_ftphandle, ftplib::dir, ftplib::ascii,result,true);
}
 ...
 * return 1 if successful, 0 otherwise
 */
int ftplib::Listing(bool full, const char *path, char *result)
{
	mp_ftphandle->offset = 0;
	return FtpXfer(NULL, path, mp_ftphandle, ftplib::dir, ftplib::ascii);
}
